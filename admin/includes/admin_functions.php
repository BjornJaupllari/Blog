...
function getAdminUsers(){
	global $conn, $roles;
	$sql = "SELECT * FROM users WHERE role IS NOT NULL AND is_deleted=0";
...
function createAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO users (username, email, role, password, cr_at, update_at) 
...
function editAdmin($admin_id)
{
	...
	$sql = "SELECT * FROM users WHERE users.user_id=$admin_id LIMIT 1";
	$result = $conn->query($sql);
...
function deleteAdmin($admin_id) {
	global $conn;
	$sql = "UPDATE users SET is_deleted=1 WHERE users.user_id=$admin_id";
	if ($conn->query($sql)) {
...
function updateCategory($request_values) {
	global $conn, $errors, $category_name, $category_id, $category_slug;
	$category_name = $request_values['category_name'];
